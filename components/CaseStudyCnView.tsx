    ...
    <>
      <div className="border-b border-gray-200 bg-white py-3">
        <div className="mx-auto max-w-7xl px-4 text-xs text-gray-400 sm:px-6 lg:px-8">
          ...
          <Link href="/case-studies" className="hover:text-[#1A56DB]">{t({ en: "Case Studies", zh: "案例研究" }, locale)}</Link><span className="mx-2">›</span>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-[1.3fr_.7fr] lg:px-8">
          <div>
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-full bg-white px-2 py-0.5 font-mono text-[11px] font-semibold text-[#1A56DB]">{t({ en: "Case Study", zh: "案例" }, locale)} {c.id}</span>
            </div>
      ...
      <section aria-label="Case metadata" className="border-b border-gray-200 bg-white">
        <div className="mx-auto grid max-w-7xl divide-y divide-gray-200 px-4 sm:grid-cols-2 sm:divide-x sm:divide-y-0 sm:px-6 lg:grid-cols-4 lg:px-8">
          {[
            [t({ en: "Industry", zh: "行业归口" }, locale), t(c.industry, locale)],
            [t({ en: "Application", zh: "应用场景" }, locale), t(c.scene, locale)],
            [t({ en: "Main Product", zh: "主推产品" }, locale), c.product],
            [t({ en: "Technology", zh: "技术路线" }, locale), t(c.techRoute, locale)],
          ].map(([label, value]) => (
      ...
      <main className="bg-white py-14">
        <div className="mx-auto grid max-w-7xl gap-12 px-4 sm:px-6 lg:grid-cols-[minmax(0,1fr)_300px] lg:px-8">
          <article className="max-w-3xl">
            <TextSection eyebrow={t({ en: "01 · Application", zh: "01 · 应用概述" }, locale)} title={t({ en: "Application Overview", zh: "应用概述" }, locale)} paragraphs={pick(c.overview, locale)} />
            <TextSection eyebrow={t({ en: "02 · Challenge", zh: "02 · 客户挑战" }, locale)} title={t({ en: "Customer Challenge", zh: "客户挑战" }, locale)} paragraphs={pick(c.challenge, locale)} />
            <TextSection eyebrow={t({ en: "03 · Solution", zh: "03 · 解决方案" }, locale)} title={t({ en: "UV Curing Solution", zh: "UV Curing 紫外线固化解决方案" }, locale)} paragraphs={pick(c.solution, locale)} />

            <section className="border-t border-gray-100 py-9">
              <p className="mb-2 text-xs font-bold uppercase tracking-widest text-[#44B549]">{t({ en: "04 · Recommended System", zh: "04 · 推荐系统" }, locale)}</p>
              <h2 className="mb-5 text-2xl font-bold text-[#1A56DB]">{t({ en: "Recommended System", zh: "推荐系统" }, locale)}</h2>
              <div className="overflow-x-auto rounded-xl border border-gray-200">
                <table className="w-full min-w-[560px] text-left text-sm">
                  <thead className="bg-[#f0f5ff] text-xs font-bold uppercase tracking-wide text-[#1A56DB]">
                    <tr>
                      <th className="px-4 py-3">{t({ en: "Process Step", zh: "工艺步骤" }, locale)}</th>
                      <th className="px-4 py-3">{t({ en: "Main Product", zh: "主推产品" }, locale)}</th>
                      <th className="px-4 py-3">{t({ en: "Value", zh: "应用价值" }, locale)}</th>
                    </tr>
                  </thead>
            ...
            <section className="border-t border-gray-100 py-9">
              <p className="mb-2 text-xs font-bold uppercase tracking-widest text-[#44B549]">{t({ en: "05 · Benefits", zh: "05 · 应用收益" }, locale)}</p>
              <h2 className="mb-5 text-2xl font-bold text-[#1A56DB]">{t({ en: "Benefits", zh: "应用收益" }, locale)}</h2>
              <div className="grid gap-3 sm:grid-cols-2">
                {pick(c.benefits, locale).map((b) => <div key={b} className="flex gap-3 rounded-lg border border-gray-100 bg-gray-50 p-4 text-sm font-medium text-gray-700"><CheckCircle2 size={18} className="shrink-0 text-[#44B549]" />{b}</div>)}
              </div>
            </section>
            ...
            <div className="my-4 flex gap-3 rounded-xl border border-[#FBBF24]/30 bg-[#FFFBEB] p-5">
              ...
              <div>
                <p className="mb-1 text-xs font-bold uppercase tracking-widest text-[#D97706]">{t({ en: "Engineer's Note", zh: "工程师提示" }, locale)}</p>
                <p className="text-[15px] leading-7 text-gray-700">{t(c.engineerTip, locale)}</p>
              </div>
            </div>

            <TextSection eyebrow={t({ en: "06 · ETIA Support", zh: "06 · ETIA 服务支持" }, locale)} title={t({ en: "ETIA Support", zh: "ETIA 服务支持" }, locale)} paragraphs={[t(c.etiaSupport, locale)]} />
          </article>

          <aside className="lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
              <p className="text-xs font-bold uppercase tracking-widest text-[#44B549]">{t({ en: "Recommended System", zh: "推荐系统" }, locale)}</p>
