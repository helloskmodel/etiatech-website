...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return caseStudyDetailMetadata(slug, "th");
}

export default async function CaseStudyThPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const cn = getCaseCnBySlug(slug);
  if (!cn) notFound();
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(caseStudyDetailJsonLd(slug, "th")) }} />
