...
const productFamilies = [
  {
    ...
    families: [
      { type: "S SERIES · LAMP-BASED · 200W HG", name: "UV Lamp Spot\nCuring Systems", models: ["S2000 Elite", "S1500 Pro", "R2000 Radiometer", "S2E Network Module", "S Series Light Guide"], bg: "#1B3D8F" },
  ...
  },
  {
    ...
    families: [
      { type: "AC LARGE · AIR-COOLED LED · >75MM WIDTH", name: "Air-Cooled UV LED\nLarge-Area Curing", models: ["ACT", "AC8", "AC8-HD", "ACN225-F", "AC UVC LED – 275nm"], bg: "#1B3D8F" },
    ...
    <>
      ...
      <div className="border-b border-gray-200 py-3 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-xs text-gray-400">
          <Link href="/product" className="hover:text-[#1B3D8F]">Products</Link>
          <span className="mx-2">›</span>
          <span style={{ color: "#1B3D8F" }}>OmniCure®</span>
        </div>
      </div>
      ...
      <section className="py-20 relative overflow-hidden" style={{ background: "linear-gradient(135deg, #0a1628 0%, #0f2444 100%)" }}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap gap-4 mb-16">
            <a href="#products" className="px-6 py-3 rounded font-semibold text-white hover:opacity-90" style={{ background: "#1B3D8F" }}>Explore Products ↓</a>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h2 className="text-3xl md:text-4xl font-bold mb-4" style={{ color: "#1B3D8F" }}>What OmniCure Delivers for You</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {benefits.map((b) => (
              <div key={b.title} className="rounded-xl p-5 border border-gray-100 hover:border-[#1B3D8F]/30 hover:shadow-md transition-all bg-gray-50">
                <div className="text-2xl mb-3">{b.icon}</div>
                <h3 className="font-semibold uppercase text-sm mb-2" style={{ color: "#1B3D8F" }}>{b.title}</h3>
      ...
      <section id="products" className="py-20" style={{ background: "#f0f4f8" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h2 className="text-3xl md:text-4xl font-bold mb-2" style={{ color: "#1B3D8F" }}>2 Technology Routes.<br />4 Product Families.</h2>
          <div className="w-12 h-1 rounded mb-10" style={{ background: "#44B549" }} />
          {productFamilies.map((group) => (
            <div key={group.category} className="mb-8">
              <span className="inline-block text-xs font-bold px-3 py-1 rounded mb-4 text-white" style={{ background: "#1B3D8F" }}>{group.category}</span>
          ...
          <div className="mt-8 text-center text-sm text-gray-500">
            ...
            <Link href="/contact" className="hover:underline font-medium" style={{ color: "#1B3D8F" }}>Talk to ETIA Engineers →</Link>
          </div>
        </div>
      </section>
