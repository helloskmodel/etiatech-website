...
const whyCards: { title: LangText; body: LangText; icon: ComponentType<{ className?: string; strokeWidth?: number }> }[] = [
  { title: { en: "20 Years of Application Experience", zh: "20 年应用经验", th: "ประสบการณ์ด้านการใช้งาน 20 ปี" }, body: { en: "Hands-on UV curing knowledge across medical, electronics, photonics, automotive and industrial manufacturing.", zh: "覆盖医疗、电子、光子、汽车及工业制造领域的 UV Curing 紫外线固化实战经验。", th: "ความรู้ด้าน UV curing เชิงปฏิบัติในอุตสาหกรรมการแพทย์ อิเล็กทรอนิกส์ โฟโตนิกส์ ยานยนต์ และการผลิตทางอุตสาหกรรม" }, icon: GraduationCap },
  { title: { en: "Authorized & Genuine Supply", zh: "授权正品供应", th: "การจัดหาสินค้าของแท้อย่างเป็นทางการ" }, body: { en: "Genuine systems, replacement lamps and accessories through authorized supply channels.", zh: "通过授权渠道供应正品系统、替换灯泡及配件。", th: "ระบบ หลอดไฟสำรอง และอุปกรณ์เสริมของแท้ผ่านช่องทางการจัดหาที่ได้รับอนุญาต" }, icon: BadgeCheck },
  { title: { en: "Local Stock & Fast Response", zh: "本地库存与快速响应", th: "สต็อกในประเทศและตอบสนองรวดเร็ว" }, body: { en: "Local equipment and consumables help reduce lead time and production risk.", zh: "本地设备与耗材库存，有助于缩短交期并降低生产风险。", th: "อุปกรณ์และวัสดุสิ้นเปลืองในประเทศช่วยลดระยะเวลารอคอยและความเสี่ยงในการผลิต" }, icon: Warehouse },
  { title: { en: "In-House Repair & Lifecycle Support", zh: "内部维修与生命周期支持", th: "การซ่อมภายในและการสนับสนุนตลอดอายุการใช้งาน" }, body: { en: "Troubleshooting, maintenance and repair coordination to keep your process running.", zh: "提供故障排查、维护与维修协调，保障工艺持续运行。", th: "การแก้ไขปัญหา การบำรุงรักษา และการประสานงานซ่อมเพื่อให้กระบวนการของคุณทำงานต่อเนื่อง" }, icon: Wrench },
];
...
export default function HomeView() {
  ...
  const scrollIndustries = (dir: number) => industryScrollRef.current?.scrollBy({ left: dir * 320, behavior: "smooth" });
  ...
  return <div className="bg-white text-[#102038]">
    <section className="relative overflow-hidden border-b border-[#D9E4EA] bg-gradient-to-br from-white via-[#EEF6FF] to-[#F1FAEF]">
      ...
      <div className="relative mx-auto grid max-w-7xl gap-12 px-4 py-16 sm:px-6 md:py-24 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:px-8">
        <div>
          ...
          <h1 className="mt-6 max-w-4xl text-4xl font-bold leading-[1.08] tracking-tight text-[#143C96] md:text-6xl"><CjkHeading locale={locale} text={{ en: "Your UV Curing Solution Partner", zh: "您的 UV Curing 紫外线固化解决方案伙伴", th: "พันธมิตรโซลูชัน UV Curing ของคุณ", vi: "Đối tác giải pháp UV Curing của bạn" }} zh={<>您的 UV Curing 紫外线固化<span className="whitespace-nowrap">解决方案伙伴</span></>} /><span className="mt-2 block text-2xl font-bold text-[#41A62A] md:text-4xl">{t({ en: "From Selection to Support.", zh: "从选型到支持", th: "ตั้งแต่การเลือกจนถึงการดูแลรักษา", vi: "Từ lựa chọn đến hỗ trợ vận hành" }, locale)}</span></h1>
    ...
    <section className="px-4 py-20 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><h2 className="text-3xl font-bold text-[#143C96] md:text-4xl">{t({ en: "Why Manufacturers Choose ETIA", zh: "为什么制造企业选择 ETIA", th: "ทำไมผู้ผลิตจึงเลือก ETIA" }, locale)}</h2><div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{whyCards.map(item=>{const Icon=item.icon;return <article key={item.title.en} className="rounded-2xl border border-[#D9E4EA] bg-white p-6 shadow-[0_10px_35px_rgba(20,60,150,.06)]"><span className="inline-flex rounded-xl bg-[#EEF6FF] p-3 text-[#143C96]"><Icon className="h-6 w-6" strokeWidth={1.7}/></span><h3 className="mt-5 font-bold text-[#143C96]">{t(item.title, locale)}</h3><p className="mt-3 text-sm leading-6 text-[#667085]">{t(item.body, locale)}</p></article>})}</div></div></section>
    ...
    <section className="bg-[#F1FAEF] px-4 py-20 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#41A62A]">{t({ en: "Manufacturing Applications", zh: "制造应用", th: "การใช้งานในการผลิต" }, locale)}</p><h2 className="mt-3 text-3xl font-bold text-[#143C96] md:text-4xl"><CjkHeading locale={locale} text={{ en: "UV Curing Application Notes by Industry", zh: "UV Curing 紫外线固化应用说明", th: "แอปพลิเคชันโน้ต UV Curing แยกตามอุตสาหกรรม", vi: "Ghi chú ứng dụng UV Curing theo ngành" }} zh={<>UV Curing 紫外线固化<span className="whitespace-nowrap">应用说明</span></>} /></h2><div ref={industryScrollRef} className="mt-10 flex snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{INDUSTRY_BLOCKS.map(block=><Link href="/applications" key={block.category} className="group relative flex aspect-square w-[210px] shrink-0 snap-start items-end overflow-hidden rounded-2xl border border-[#D9E4EA] bg-[#EEF6FF] shadow-[0_10px_35px_rgba(20,60,150,.06)] transition hover:-translate-y-1 hover:shadow-lg sm:w-[240px]">{block.image&&<Image src={block.image} alt="" fill sizes="240px" className="object-cover transition duration-500 group-hover:scale-105"/>}<div className="absolute inset-0 bg-gradient-to-t from-[#0B1E3F]/85 via-[#0B1E3F]/25 to-transparent"/><div className="relative flex w-full items-center gap-1.5 p-4"><span className="text-sm font-bold leading-snug text-white">{t(block.label, locale)}</span><ArrowRight className="ml-auto h-4 w-4 shrink-0 text-white/90 transition group-hover:translate-x-0.5"/></div></Link>)}</div><div className="mt-6 flex justify-end gap-3"><button type="button" aria-label="Previous industries" onClick={()=>scrollIndustries(-1)} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#D4DFEC] bg-white text-[#143C96] transition hover:border-[#143C96] hover:text-[#1F63D6]"><ChevronLeft className="h-5 w-5"/></button><button type="button" aria-label="Next industries" onClick={()=>scrollIndustries(1)} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#D4DFEC] bg-white text-[#143C96] transition hover:border-[#143C96] hover:text-[#1F63D6]"><ChevronRight className="h-5 w-5"/></button></div></div></section>

    <section className="bg-gradient-to-br from-[#143C96] to-[#1F63D6] px-4 py-20 text-white sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><div className="flex items-end justify-between gap-4"><div><h2 className="text-3xl font-bold md:text-4xl"><CjkHeading locale={locale} text={{ en: "UV Curing Success Stories", zh: "UV Curing 紫外线固化案例分享", th: "เรื่องราวความสำเร็จ UV Curing", vi: "Câu chuyện thành công UV Curing" }} zh={<>UV Curing 紫外线固化<span className="whitespace-nowrap">案例分享</span></>} /></h2></div><Link href="/applications" className="hidden items-center gap-2 text-sm font-bold text-white sm:inline-flex">{t({ en: "View Applications", zh: "查看应用", th: "ดูการใช้งาน" }, locale)} <ArrowRight className="h-4 w-4"/></Link></div><div className="mt-10"><div ref={caseScrollRef} className="flex snap-x snap-mandatory gap-5 overflow-x-auto scroll-smooth pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{caseStudiesCn.map(c=><Link href={`/case-studies/${c.slug}`} key={c.slug} className="group relative flex aspect-[3/4] w-[290px] shrink-0 snap-start items-end overflow-hidden rounded-2xl bg-[#0B1E3F] shadow-xl">{c.image&&<Image src={c.image} alt={t(c.title, locale)} fill sizes="290px" className="object-cover transition duration-500 group-hover:scale-105"/>}<div className="absolute inset-0 bg-gradient-to-t from-[#0B1E3F] via-[#0B1E3F]/60 to-[#0B1E3F]/5"/><div className="relative p-5"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#63C94A]">{t(c.industry, locale)} · {c.product}</p><h3 className="mt-2 line-clamp-4 text-lg font-bold leading-snug text-white">{t(c.title, locale)}</h3><span className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-white/90">{t({ en: "Read case", zh: "阅读案例", th: "อ่านกรณีศึกษา", vi: "Xem case study" }, locale)} <ArrowRight className="h-3.5 w-3.5 transition group-hover:translate-x-0.5"/></span></div></Link>)}</div><div className="mt-6 flex justify-end gap-3"><button type="button" aria-label="Previous case studies" onClick={()=>scrollCases(-1)} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/40 bg-white/10 text-white transition hover:bg-white/20"><ChevronLeft className="h-5 w-5"/></button><button type="button" aria-label="Next case studies" onClick={()=>scrollCases(1)} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/40 bg-white/10 text-white transition hover:bg-white/20"><ChevronRight className="h-5 w-5"/></button></div></div></div></section>
