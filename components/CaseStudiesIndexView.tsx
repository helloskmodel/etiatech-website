    ...
    <>
      <header className="relative overflow-hidden border-b border-[#D9E4EA] bg-gradient-to-br from-white via-[#EEF6FF] to-[#F1FAEF]">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 md:py-20 lg:px-8">
          <div className="inline-flex items-center gap-2 rounded-full border border-[#41A62A]/20 bg-white px-3 py-1.5 text-xs font-bold text-[#41A62A] shadow-sm"><BadgeCheck className="h-4 w-4" /> {t({ en: "Case Studies", zh: "案例研究" }, locale)}</div>
          <h1 className="mt-6 max-w-3xl text-4xl font-bold leading-[1.08] tracking-tight text-[#143C96] md:text-5xl">{t({ en: "UV Curing Case Studies", zh: "UV Curing 紫外线固化案例研究" }, locale)}</h1>
          <p className="mt-5 max-w-3xl text-base leading-7 text-[#667085]">{t({ en: "How OmniCure systems solve specific production challenges — one industry, one product, one clear result.", zh: "OmniCure 系统如何解决具体的生产问题——一个行业、一个主推产品、一个明确结果。" }, locale)}</p>
        </div>
      </header>
            ...
            {caseStudiesCn.map((c) => (
              <Link key={c.slug} href={`/case-studies/${c.slug}`} className="group flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white transition-all hover:border-[#1A56DB]/30 hover:shadow-md">
                ...
                <div className="flex flex-1 flex-col border-t border-gray-100 p-5">
                  ...
                  <p className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-[#1A56DB]">{t({ en: "Read case study", zh: "查看案例" }, locale)} <ArrowRight className="h-3.5 w-3.5" /></p>
                </div>
              </Link>
            ))}
          </div>
