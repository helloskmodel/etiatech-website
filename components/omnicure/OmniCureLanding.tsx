...
import { BRAND, CONTACT, getCopy, type Lang, type FormLang, type LandingCopy } from "./copy";
...
// Country-agnostic OmniCure SEM landing. Defaults render the original
// Thailand page; the Vietnam pages pass their own copy/contact/labels.
// LINE CTAs only render when the contact actually has a LINE id.
export default function OmniCureLanding({
  lang,
  copy,
  contact = CONTACT,
  regionLabel = "OmniCure · Thailand",
  footerLabel = "Authorized OmniCure Distributor in Thailand",
  pageId,
}: {
  lang: FormLang;
  copy?: LandingCopy;
  ...
  const c = copy ?? getCopy(lang as Lang);
  const page = pageId ?? `omnicure-${lang}`;
  const lineUrl = contact.lineId && contact.lineId !== "TODO_FILL" ? `https://line.me/R/ti/p/${contact.lineId}` : contact.lineId === "TODO_FILL" ? "#line" : "";
  ...
  return (
    <div className="pb-20 md:pb-0">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 grid md:grid-cols-2 gap-10 items-start">
          <div>
            ...
            <div className="hidden md:flex flex-wrap gap-3">
              ...
              {lineUrl && (
                <a href={lineUrl} onClick={onLine} className="rounded-lg px-6 py-3 text-sm font-bold text-white border border-white/40 hover:border-white/70">
                  {c.hero.ctaSecondary}
                </a>
              )}
            </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-14 grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {lineUrl && (
                <a href={lineUrl} onClick={onLine} className="rounded-lg px-5 py-3 text-sm font-bold text-white border border-white/40 hover:border-white/70">
                  💬 LINE
                </a>
              )}
            </div>
      ...
      <div className={`md:hidden fixed bottom-0 inset-x-0 z-50 grid ${lineUrl ? "grid-cols-2" : "grid-cols-1"} border-t border-gray-200 bg-white`}>
        ...
        {lineUrl && (
          <a href={lineUrl} onClick={onLine} className="py-3.5 text-center text-sm font-bold text-white" style={{ background: BRAND.green }}>
            💬 LINE
          </a>
        )}
      </div>
      ...
      <footer className="bg-gray-900 text-gray-400">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-xs flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <span>© {new Date().getFullYear()} ETIA · {footerLabel}</span>
          </div>
