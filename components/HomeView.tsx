...
const services: { title: LangText; body: LangText; icon: ComponentType<{ className?: string; strokeWidth?: number }> }[] = [
  { title: { en: "Genuine Product Assurance", zh: "正品保障", th: "การรับประกันสินค้าของแท้" }, body: { en: "Authorized supply of genuine systems, lamps and accessories.", zh: "授权供应正品系统、灯管与配件。", th: "การจัดหาระบบ หลอดไฟ และอุปกรณ์เสริมของแท้อย่างเป็นทางการ" }, icon: ShieldCheck },
  { title: { en: "Application-Driven Selection", zh: "以应用为导向的选型", th: "การเลือกที่ขับเคลื่อนด้วยการใช้งาน" }, body: { en: "Selection based on material, adhesive, geometry, wavelength and cycle time.", zh: "基于材料、胶粘剂、几何结构、波长与节拍时间进行选型。", th: "การเลือกตามวัสดุ กาว รูปทรง ความยาวคลื่น และรอบเวลา" }, icon: Crosshair },
  { title: { en: "Installation & Training", zh: "安装与培训", th: "การติดตั้งและการฝึกอบรม" }, body: { en: "Setup guidance, operator training and production implementation support.", zh: "提供安装指导、操作员培训与生产导入支持。", th: "คำแนะนำการติดตั้ง การฝึกอบรมผู้ปฏิบัติงาน และการสนับสนุนการนำไปใช้ในการผลิต" }, icon: PackageCheck },
  { title: { en: "Troubleshooting & Repair", zh: "故障排查与维修", th: "การแก้ไขปัญหาและการซ่อม" }, body: { en: "Local diagnosis, maintenance, repair and lifecycle coordination.", zh: "提供本地诊断、维护、维修与全生命周期协调。", th: "การวินิจฉัยในประเทศ การบำรุงรักษา การซ่อม และการประสานงานตลอดอายุการใช้งาน" }, icon: Wrench },
];
...
export default function HomeView() {
  ...
  return <div className="bg-white text-[#102038]">
    <section className="relative overflow-hidden border-b border-[#D9E4EA] bg-gradient-to-br from-white via-[#EEF6FF] to-[#F1FAEF]">
      ...
      <div className="relative mx-auto grid max-w-7xl gap-12 px-4 py-16 sm:px-6 md:py-24 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:px-8">
        <div>
          ...
          <h1 className="mt-6 max-w-4xl text-4xl font-bold leading-[1.08] tracking-tight text-[#143C96] md:text-6xl">{t({ en: "Your UV Curing Solution Partner ", zh: "您的 UV 固化解决方案伙伴", th: "พันธมิตรโซลูชัน UV Curing ของคุณ" }, locale)}<span className="text-[#41A62A]">{t({ en: "— From Selection to Support.", zh: "——从选型到支持。", th: "— ตั้งแต่การเลือกจนถึงการสนับสนุน" }, locale)}</span></h1>
          <p className="mt-4 max-w-2xl text-lg font-semibold leading-snug text-[#41A62A] md:text-xl">{t({ en: "Authorized systems, application engineering, and local service across China and Southeast Asia.", zh: "授权系统、应用工程与本地服务，覆盖中国与东南亚。", th: "ระบบที่ได้รับอนุญาต วิศวกรรมการใช้งาน และบริการในประเทศทั่วจีนและเอเชียตะวันออกเฉียงใต้" }, locale)}</p>
    ...
    <section className="px-4 py-20 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#41A62A]">{t({ en: "Long-Term Support", zh: "长期支持", th: "การสนับสนุนระยะยาว" }, locale)}</p><h2 className="mt-3 text-3xl font-bold text-[#143C96] md:text-4xl">{t({ en: "ETIA Service Commitment", zh: "ETIA 服务承诺", th: "พันธสัญญาการบริการของ ETIA" }, locale)}</h2><p className="mt-4 max-w-3xl text-[#667085]">{t({ en: "Genuine products. Application-driven solutions. Local technical support. Long-term service.", zh: "正品产品。以应用为导向的解决方案。本地技术支持。长期服务。", th: "สินค้าของแท้ โซลูชันที่ขับเคลื่อนด้วยการใช้งาน การสนับสนุนทางเทคนิคในประเทศ บริการระยะยาว" }, locale)}</p><div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{services.map(item=>{const Icon=item.icon;return <article key={item.title.en} className="rounded-2xl border border-[#D9E4EA] bg-white p-6"><span className="inline-flex rounded-xl bg-[#F1FAEF] p-3 text-[#41A62A]"><Icon className="h-6 w-6" strokeWidth={1.7}/></span><h3 className="mt-5 font-bold text-[#143C96]">{t(item.title, locale)}</h3><p className="mt-3 text-sm leading-6 text-[#667085]">{t(item.body, locale)}</p></article>})}</div></div></section>
